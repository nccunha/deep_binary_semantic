...
class SegNet(nn.Module):
    ...
    def forward(self, rgb, ndvi):
        ...
        for i in range(0, 5):
            (feat_rgb, ind), size = self.encoders[i](feat_rgb)
            indices_rgb.append(ind)
            unpool_sizes_rgb.append(size)
            feat_vec_rgb.append(feat.detach())
        for i in range(0, 5):
            (feat_ndvi, ind_ndvi), size_ndvi = self.encoders[i](feat_ndvi)
            indices_rgb.append(ind_ndvi)
            unpool_sizes_rgb.append(size_ndvi)
            feat_vec_ndvi.append(feat.detach())


        fused_feat = (torch.mean(torch.stack(feat_vec_rgb), dim=0) + torch.mean(torch.stack(feat_vec_ndvi), dim=0)) / 2.0

        # invert order
        feat_vec = fused_feat[::-1]
